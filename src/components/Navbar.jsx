...
if(isMobile){
  return <nav className='relative w-full h-[9vh] md:h-[8vh] fixed top-0 px-10 md:px-20 bg-[#E5BA73] flex justify-between items-center shadow-[3px_0px_10px_1px_rgba(0,0,0,0.40)]'>
    ...
    <>
      <aside className={`absolute h-screen w-[49%] ${isOpen ? 'translate-x-[0px] duration-300 transiton-all ease-in-out' : 'translate-x-[-500px] duration-500 transiton-all ease-in'} flex flex-col justify-between items-center pt-2 md:pt-4 pb-10 md:pb-20 top-0 left-0 bg-[#E5BA73] shadow-[3px_0px_10px_1px_rgba(0,0,0,0.40)] z-50`}>
        ...
        <ul>
        {menuList.map((data)=>{
          return <li key={data.title} onClick={()=>handleClick(data.event)} className='group text-[19px] md:text-[32px] text-white font-[600] my-5 md:my-10 px-3 py-1 flex justify-center items-center gap-4 border-[3.1px] md:border-[4.5px] rounded-md md:rounded-lg hover:bg-slate-200 last:mt-40 md:last:mt-60 last:border-none last:bg-[#C58940] last:p-2 md:last:p-3 relative' >{data.title}{data.icon&&<><span className='text-[22px]'>{data.icon}</span><section className={`invisible group-hover:visible absolute w-full top-9 md:top-14 bg-[#FAF8F1]`}>{data.submenu.map(sub => {return <span key={sub.title} className='text-[18px] md:text-[30px] hover:bg-slate-200 block w-full' onClick={()=>alert('yes')}>{sub.title}</span>})}</section></>}</li>
        })
        }
        </ul>
      </aside>
  ...
  return <nav className='w-full h-[10vh] bg-[#E5BA73] fixed flex items-center justify-between px-20'>
    <section className='w-[5vw] bg-slate-200 rounded-full p-2'>
      <img src={BrandImgShort} alt="Logo MIM - Menghafal Itu Mudah"/>
    </section>
    <section>
      <ul className='flex'>
        {menuList.map((data)=>{
          return <li key={data.title} onClick={()=>handleClick(data.event)} className='group text-[20px] text-white font-[500] mx-4 px-4 py-1 border-[3.8px] rounded-xl last:border-none last:bg-[#C58940] last:ml-[143px] cursor-pointer flex justify-center items-center relative' >{data.title}{data.icon&&<><span className='text-[22px] ml-4'>{data.icon}</span><section className={`invisible group-hover:visible rounded-b-md absolute w-full top-[38px] bg-[#C58940] -z-10`}>{data.submenu.map(sub => {return <span key={sub.title} className='text-[20px] p-2 hover:bg-slate-200 block w-full' onClick={()=>alert('yes')}>{sub.title}</span>})}</section></>}</li>
        })
        }
      </ul>
    </section>
