      ...
      <main>
        ...
        <h2 className='text-[22px] text-white bg-[#FAEAB1] md:bg-transparent p-1 rounded-t-lg md:rounded-t-none md:absolute md:m-auto md:-left-5 lg:left-7 md:top-0 md:bottom-0 h-fit w-fit md:rotate-[-90deg] md:text-[30px] z-40'>Fitur Utama</h2>
          <section className='relative flex gap-2 md:gap-5 px-5 md:pl-10 md:mx-[40px] lg:mx-[85px] md:rounded-xl bg-gradient-to-r from-[#FAEAB1] to-[#FFFFFF] shadow-[inset_0px_0px_10px_rgba(0,0,0,0.6)]'>
            ...
            <div key={id} onClick={()=>handleClick(id)} className={`relative ${isActive == id ? 'flex-[1.1] md:flex-[1.8] lg:flex-[1.5]' : 'flex-[0.2] md:flex-[0.9] lg:flex-[0.9]'} flex items-center justify-center mx-auto w-full md:h-[400px] h-[80vh] transition-[flex] duration-700 ease-out-flex cursor-pointer`}>
                {playVid == id ? (<><img src={data.gif} alt={data.title} className='absolute w-full h-full object-cover'/>
                <span className='w-full bottom-0 absolute bg-black/30 text-white text-center z-10'><h2 className='text-[20px] font-bold my-4'>{data.title}</h2><p className='text-[17px]'>{data.description}</p></span></>) : (<><img src={data.img} alt={data.title} className='h-full object-cover'/>
