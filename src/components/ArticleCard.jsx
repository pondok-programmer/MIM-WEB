...
const ArticleCard = ({img,title,path}) => {
  return (
    <section className="relative rounded-lg p-[7px] w-[330px] h-fit bg-[#FAEAB1] shadow-[0px_0px_10px_rgba(0,0,0,0.3)]">
        ...
        <section className="absolute bottom-0 text-white w-full flex z-10">
            ...
            <Link to={`/artikel/${path}`}><ButtonCustom value={'Baca'} className={'flex-[1] py-[6px] px-3 text-[20px] font-bold rounded-lg rounded-br-none bg-white/70 w-full'}/></Link>
        </section>
        </section>
    </section>
  )
